...
- `MailConfigFromEnv() MailConfig` — maps `EMAIL_SERVICE=Gmail` (or `Google`) to `smtp.gmail.com:587`; otherwise honors `EMAIL_HOST`/`EMAIL_PORT`. Trims the address; `FromName` is `"Grip"`.
